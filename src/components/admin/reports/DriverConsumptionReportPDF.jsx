...
export default function DriverConsumptionReportPDF({
  reportData,
  startDate,
  endDate,
}) {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        {/* Encabezado */}
        <Text style={styles.header}>Reporte de Consumo por Chofer</Text>
        ...
        <View style={{ marginBottom: 8 }}>
          <Text style={styles.kpiItem}>
            <Text style={styles.kpiLabel}>• Total de Choferes: </Text>
            <Text style={styles.kpiValue}>{reportData.totalDrivers || 0}</Text>
          </Text>
          <Text style={styles.kpiItem}>
        ...
        {/* RESUMEN COMPARATIVO */}
        <Text style={styles.sectionTitle}>RESUMEN POR CHOFER:</Text>
        <View style={styles.sectionLine} />

        <View style={styles.tableHeader}>
          <Text style={[styles.tableHeaderText, { flex: 2 }]}>Chofer</Text>
          <Text style={styles.tableHeaderText}>Viajes</Text>
          <Text style={styles.tableHeaderText}>Vehículos</Text>
        ...
        {reportData.drivers &&
          reportData.drivers.map((driver) => {
            const difference = driver.difference || 0;
            const diffStyle =
              difference > 0
            ...
            return (
              <View style={styles.tableRow} key={driver.driverId}>
                ...
                <Text style={styles.tableCell}>{driver.trips || 0}</Text>
                <Text style={styles.tableCell}>{driver.vehicles || 0}</Text>
                <Text style={styles.tableCell}>
                  {(driver.estimated || 0).toFixed(2)}
                </Text>
                <Text style={styles.tableCell}>
                  {(driver.actual || 0).toFixed(2)}
                </Text>
