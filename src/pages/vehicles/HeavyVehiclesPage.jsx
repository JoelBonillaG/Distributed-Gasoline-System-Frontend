...
import { Plus, Pencil, Trash2, Loader2, Truck, Eye, FileText, CheckCircle, XCircle, Calendar } from "lucide-react";
...
export default function HeavyVehiclesPage() {
  ...
  // Cargar vehículos pesados
  const loadVehicles = React.useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await vehiclesService.getAllVehicles("HEAVY");
      setVehicles(data || []);
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeading
        title="Modelos de Vehículos Pesados"
        subtitle="Administra los modelos de vehículos pesados del sistema"
        icon={Truck}
        actions={
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <CardTitle className="text-base">Modelos de Vehículos Pesados</CardTitle>
                <Badge variant="outline" className="bg-chart-2/20 text-chart-2 hover:bg-chart-2/30">
                  Pesada
                </Badge>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">
                Listado de todos los modelos pesados registrados. Haz clic en una fila para ver detalles.
              </p>
            </div>
