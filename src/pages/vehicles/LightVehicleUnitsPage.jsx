...
export default function LightVehicleUnitsPage() {
  ...
  // Cargar unidades y modelos livianos
  const loadData = React.useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const [unitsData, modelsData] = await Promise.all([
        vehiclesService.getAllUnits("LIGHT"),
        vehiclesService.getAllVehicles("LIGHT"),
      ]);
      setUnits(unitsData || []);
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeading
        title="Unidades de Vehículos Livianos"
        subtitle="Administra las unidades individuales de vehículos livianos del sistema"
        icon={Gauge}
        actions={
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <CardTitle className="text-base">Unidades de Vehículos Livianos</CardTitle>
                <Badge variant="outline" className="bg-chart-1/20 text-chart-1 hover:bg-chart-1/30">
                  Liviana
                </Badge>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">
                Listado de todas las unidades livianas registradas en el sistema.
              </p>
            </div>
