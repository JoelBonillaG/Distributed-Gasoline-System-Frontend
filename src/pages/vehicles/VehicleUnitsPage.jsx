...
export default function VehicleUnitsPage() {
  ...
  // Cargar unidades y modelos
  const loadData = React.useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const [unitsData, modelsData] = await Promise.all([
        vehiclesService.getAllUnits(),
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeading
        title="Unidades de Vehículos"
        subtitle="Administra las unidades individuales de vehículos del sistema"
        icon={Gauge}
        actions={
      ...
      <Card className="overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base">Unidades de Vehículos</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Listado de todas las unidades registradas en el sistema.
              </p>
            </div>
